...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  //新增Memo时把isRead2的字段写个"no"(空字符),表示后台有新的memo; cMemo写个yes,表示该job有memo存在.
  $updateSQL = sprintf("UPDATE tbrepair SET cMemo='yes', cIsReplyRead2='', cIsReplyRead='checked' WHERE cJN=%s",
                       GetSQLValueString($_POST['cJN'], "text"));
...
//打开页面时把isRead的字段写个"checked",表示后台已经读过memo
mysql_select_db($database_localhost, $localhost);
$updateSQL = sprintf("UPDATE tbrepair SET cIsReplyRead='checked' WHERE cJN=%s",$colname_mm);
//echo $updateSQL;
