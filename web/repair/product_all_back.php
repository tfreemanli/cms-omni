...
if(isset($_GET['job_num']) && $_GET['job_num']!=""){
	mysql_select_db($database_localhost, $localhost);
	$sql_query = "select product_id, quantity from stock_stockout where job_num='".$_GET['job_num']."'";
	...
	if($totalRows_rs>0){
		...
		$sql_out = "delete from stock_stockout where job_num='".$_GET['job_num']."'";
		mysql_query($sql_out, $localhost) or die(mysql_error());
		
		mysql_select_db($database_localhost, $localhost);
		  $logperson = $_SESSION['RP_WHOAMI'];
		  $logcontent = "All parts back to Stock";
