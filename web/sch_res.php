...
$str="";
if(isset($_POST['sch_val']) && ltrim(rtrim($_POST['sch_val']))!=""){
	$str = eregi_replace("3G","checked",ltrim(rtrim($_POST['sch_val'])));
	$str = "+". $str ."*";//str will look like '+foo boo foo*'
	$str = str_replace(" ","* >",$str);// str will look like '+foo* >boo* >foo*'
}
?>
  ...
  <tr valign="top">
    ...
    <td width="593" rowspan="2" bgcolor="#FFFFFF">
      ...
      <table width="530" border="0" align="center" cellpadding="2" cellspacing="0">
        ...
        <tr>
          <td colspan="2"><span class="head_red_bold">Search Result </span><?php //echo $str;?></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
		...
		?>
			<?php
			$query_RS = "SELECT iID, cBrand,cModel,cFeature,cImg FROM tbphone WHERE MATCH (cBrand,cIs3G,cModel,cFeature) AGAINST ('".$str."' IN boolean MODE) limit 0,20";
			//echo $query_RS;
			$RS = mysql_query($query_RS, $localhost) or die(mysql_error());
			while($row_RS = mysql_fetch_assoc($RS)){
				//$replace="<span class=head_red_bold>".$str."</span>";
			?>
			<tr bgcolor="#E3E3E3">
			  <td colspan="2" class="STYLE1"><img width="5" height="5" hspace="3" style="background-color: #FF8000"> <a href="./ph_detail.php?id=<?php echo $row_RS['iID'];?>"><?php echo $row_RS['cBrand'];?> <?php echo $row_RS['cModel'];?></a></td>
		</tr>
			<tr>
			  <td align="center" valign="top"><a href="./ph_detail.php?id=<?php echo $row_RS['iID']; ?>"><img src="./productimg/<?php echo $row_RS['cImg']; ?>" width="70" height="93" hspace="5" vspace="5" border="0" class="PhImg"></a></td>
			  <td valign="top"><?php echo substr(str_replace("\n","<br>",$row_RS['cFeature']),0,300);?>...</td>
			</tr>
			<?php
			}
			?>
			<?php
			//$query_RS = "SELECT * FROM tbanm where cBrand like '%".$str."%' or cModel like '%".$str."%' or cDesc like '%".$str."%' ORDER BY iAorM, iShowIndex desc, iID desc";
			$query_RS = "SELECT * FROM tbanm WHERE MATCH (cBrand,cModel,cDesc) AGAINST ('".$str."' IN boolean MODE) limit 0,10";
			//echo $query_RS;
			$RS = mysql_query($query_RS, $localhost) or die(mysql_error());
			while($row_RS = mysql_fetch_assoc($RS)){
				$type = "ac";
				if($row_RS['iAorM']=1) $type="mp";
				if($row_RS['iAorM']=3) $type="event";
			?>
			<tr bgcolor="#e3e3e3">
			  <td colspan="2" class="STYLE1"><img width="5" height="5" hspace="3" style="background-color: #FF8000"> <a href="./<?php echo $type;?>_detail.php?iID=<?php echo $row_RS['iID'];?>"><?php echo $row_RS['cPC']; ?></a></td>
		</tr>
			<tr>
			  <td align="center" valign="top"><a href="./<?php echo $type;?>_detail.php?iID=<?php echo $row_RS['iID']; ?>"><img src="./productimg/<?php echo $row_RS['cImg']; ?>" width="70" height="93" hspace="5" vspace="5" border="0" class="PhImg"></a></td>
			  <td valign="top"><?php echo substr(str_replace("\n","<br>",$row_RS['cDesc']),0,300);?>...</td>
			</tr>
			<?php
			}
			?>
        ...
        <tr>
          <td>&nbsp;</td>
          <td>Sorry, no matched result for your search.</td>
        </tr>
		<?php
		}
		?>
